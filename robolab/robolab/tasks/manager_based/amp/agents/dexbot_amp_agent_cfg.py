...
from robolab.tasks.manager_based.amp.mdp.symmetry import dexbot
...
class RslRlOnPolicyRunnerAmpCfg(RslRlOnPolicyRunnerCfg):
    ...
    max_iterations = 5000
    save_interval = 200
    experiment_name = "dexbot_amp"
    wandb_project = "dexbot_amp"
    obs_groups = {
        "policy": ["policy"], 
        "critic": ["critic"], 
        "discriminator": ["disc"],
    ...
    algorithm = RslRlPpoAmpAlgorithmCfg(
        ...
        symmetry_cfg=RslRlSymmetryCfg(
            ...
            data_augmentation_func=dexbot.compute_symmetric_states
        ),
        amp_cfg=RslRlAmpCfg(
